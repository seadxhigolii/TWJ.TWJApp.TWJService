...
namespace TWJ.TWJApp.TWJService.Application.Services.Base.Commands.Update
{
    public class UpdateTemplateSettingCommandHandler : IRequestHandler<UpdateTemplateSettingCommand, Unit>
    {
        ...
        {
            var data = await _context.TemplateSettings.AsNoTrackingWithIdentityResolution().FirstOrDefaultAsync(x => x.Name == request.Name, cancellationToken);

            _context.TemplateSettings.Update(request.UpdateTemplateSetting(data));
