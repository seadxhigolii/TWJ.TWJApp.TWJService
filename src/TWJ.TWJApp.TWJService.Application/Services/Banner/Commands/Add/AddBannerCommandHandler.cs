...
{
    public class AddBannerCommandHandler : IRequestHandler<AddBannerCommand, Unit>
    {
        private readonly ITWJAppDbContext _context;
        private readonly IGlobalHelperService _globalHelper;
        private readonly string currentClassName = "";

        public AddBannerCommandHandler(ITWJAppDbContext context, IGlobalHelperService globalHelper)
        {
        ...
        public async Task<Unit> Handle(AddBannerCommand request, CancellationToken cancellationToken)
        {
            try
            {
                await _context.Banners.AddAsync(request.AddBanner(), cancellationToken);
